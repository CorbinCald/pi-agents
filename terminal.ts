...
async function clipboardCopyCommand(
	environment: NodeJS.ProcessEnv,
): Promise<string | undefined> {
	if (
		environment.SSH_CONNECTION ||
		environment.SSH_CLIENT ||
		environment.MOSH_CONNECTION
	) {
		return undefined;
	}
...
async function configureClipboard(
	server: string,
	environment: NodeJS.ProcessEnv,
): Promise<void> {
	await execFileAsync(
		"tmux",
		["-L", server, "set-option", "-s", "set-clipboard", "external"],
		{ env: environment },
	);
...
async function configureCopyMode(
	server: string,
	environment: NodeJS.ProcessEnv,
): Promise<void> {
