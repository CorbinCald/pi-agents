...
function detachClient(): void {
	...
	execFile(
		"tmux",
		["-L", TMUX_SERVER, "detach-client", "-s", process.env.PI_AGENTS_TMUX_SESSION || ""],
		() => undefined,
	);
}
...
function recapComponent(job: AgentRecord) {
	return (_tui: unknown, theme: ExtensionContext["ui"]["theme"]) => {
		...
		container.addChild(
			new Text(`${theme.bold(theme.fg("accent", "Agent recap"))}\n${job.recap || job.summary}`, 1, 0),
		);
		container.addChild(new DynamicBorder((text: string) => theme.fg("accent", text)));
		return container;
	};
...
export function registerWorkerIntegration(pi: ExtensionAPI): void {
	...
	const applyJobState = (job: AgentRecord) => {
		...
		ctx.ui.setStatus(
			STATUS_KEY,
			ctx.ui.theme.fg(color, `Agents · ${job.status.replace("_", " ")} · ← or /agents to detach`),
		);
	...
	const sendEvent = async (eventType: string, data: Record<string, unknown> = {}) => {
		try {
	...
	const updateReasoningStatus = () => {
		...
		ctx.ui.setStatus(REASONING_STATUS_KEY, maxPro ? ctx.ui.theme.fg("accent", "Max Pro") : undefined);
	};
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		installAgentsNavigationEditor(
			...
			(keybindings) => {
				unsubscribeExit?.();
				unsubscribeExit = installSinglePressExit(ctx, keybindings);
			},
		);
		...
		try {
			...
			ctx.ui.setStatus(STATUS_KEY, ctx.ui.theme.fg("warning", "Agents · supervisor reconnecting · ← detach"));
		}
	});

	pi.on("before_provider_request", (event, ctx) => {
		if (reasoning.getSelection().reasoningMode !== "pro" || !supportsMaxProReasoning(ctx.model)) {
			return;
		}
	...
	pi.on("session_shutdown", async (event, ctx) => {
		...
		ctx.ui.setWidget(RECAP_WIDGET, undefined);
		if (event.reason === "quit") await sendEvent("session_shutdown", { reason: event.reason });
		unsubscribe?.();
