...
class AgentsNavigationEditor implements EditorComponent {
	...
	constructor(
		...
		this.onThinkingCycle = onThinkingCycle;
	...
	private getAppAwareBase(): AppAwareEditor | undefined {
		if ("actionHandlers" in this.base && (this.base as { actionHandlers?: unknown }).actionHandlers instanceof Map) {
	...
	handleInput(data: string): void {
		if (this.keybindings.matches(data, "app.thinking.cycle") && this.onThinkingCycle?.()) {
			return;
		}
		...
		if (this.keybindings.matches(data, "tui.editor.cursorLeft") && this.base.getText().length === 0) {
			this.onEmptyLeft();
			return;
		}
		...
		if (this.keybindings.matches(data, "app.interrupt")) {
			const showingAutocomplete =
				...
				typeof (this.base as { isShowingAutocomplete?: () => boolean }).isShowingAutocomplete === "function" &&
				(this.base as { isShowingAutocomplete: () => boolean }).isShowingAutocomplete();
...
export function installAgentsNavigationEditor(
	...
	createDefault: (tui: TUI, theme: EditorTheme, keybindings: KeybindingsManager) => EditorComponent,
	onEmptyLeft: (editor: AgentsNavigationEditor) => void,
	onThinkingCycle?: () => boolean,
	onSuspend?: () => void,
	onInstalled?: (keybindings: KeybindingsManager) => void,
): void {
	const previousFactory = context.ui.getEditorComponent();
	context.ui.setEditorComponent((tui, theme, keybindings) => {
		onInstalled?.(keybindings);
		const base = previousFactory?.(tui, theme, keybindings) ?? createDefault(tui, theme, keybindings);
		let editor: AgentsNavigationEditor;
		editor = new AgentsNavigationEditor(base, keybindings, () => onEmptyLeft(editor), onThinkingCycle, onSuspend);
		return editor;
	});
}
