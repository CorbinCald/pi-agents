...
const LEGACY_SESSIONS_DIR = join(ROOT, "sessions");
const SESSION_ROOT =
	process.env.PI_AGENTS_SESSION_ROOT || join(dirname(ROOT), "sessions");
...
function supportsMaxProReasoning(model) {
	return (
		...
		try {
			...
			if (parsed.hidden) {
				if (typeof parsed.sessionFile === "string") {
					hiddenSessionFiles.add(resolve(parsed.sessionFile));
				}
				if (typeof parsed.sessionId === "string") {
					hiddenSessionIds.add(parsed.sessionId);
				}
				continue;
			}
...
function terminalLaunchCommand(job, initialPrompt) {
	...
	if (job.model?.provider && job.model?.id) {
	...
	if (job.projectTrusted === true) args.push("--approve");
	else if (job.projectTrusted === false) args.push("--no-approve");
...
function isLegacySessionFile(filePath) {
	if (typeof filePath !== "string") return false;
	const relativePath = relative(
		resolve(LEGACY_SESSIONS_DIR),
		resolve(filePath),
	);
	return (
		relativePath !== "" &&
...
async function migrateLegacySessions() {
	...
	for (const job of jobs.values()) {
		if (!isLegacySessionFile(job.sessionFile)) continue;
		if (await hasTerminalSession(job)) await stopTerminal(job);
	}

	const migrated = migrateLegacySessionFiles(LEGACY_SESSIONS_DIR, SESSION_ROOT);
	if (migrated.size === 0) return;
	const targetBySessionId = new Map();
	for (const [sourcePath, targetPath] of migrated) {
		const session = readNativeSession(targetPath);
		if (session) targetBySessionId.set(session.id, targetPath);
		if (hiddenSessionFiles.delete(sourcePath))
			hiddenSessionFiles.add(targetPath);
	}
	for (const job of jobs.values()) {
		const sourcePath =
			typeof job.sessionFile === "string"
				? resolve(job.sessionFile)
				: undefined;
		const targetPath =
			(sourcePath ? migrated.get(sourcePath) : undefined) ||
			(typeof job.sessionId === "string"
				? targetBySessionId.get(job.sessionId)
				: undefined);
		if (!targetPath) continue;
		job.sessionFile = targetPath;
		persistJob(job);
	}
	log(
		`Migrated ${migrated.size} legacy Pi session${migrated.size === 1 ? "" : "s"} into ${SESSION_ROOT}`,
	);
}
	...
	switch (eventType) {
		case "session_start":
			if (typeof data.sessionFile === "string")
				job.sessionFile = resolve(data.sessionFile);
...
function discoveredJobId(sessionPath) {
...
function syncDiscoveredSessions() {
	...
	for (const job of jobs.values()) {
		if (typeof job.sessionFile === "string")
			byPath.set(resolve(job.sessionFile), job);
	...
	for (const sessionFile of listNativeSessionFiles(SESSION_ROOT)) {
		...
		if (job) {
			if (
				...
				!existsSync(job.sessionFile || "")
			) {
				job.sessionFile = session.path;
			}
			if (job.discovered && !terminalRunning.has(job.id)) {
				job.originalCwd = session.cwd;
				job.cwd = session.cwd;
				job.prompt = session.prompt;
				job.summary = session.summary;
				job.model = session.model;
				job.thinkingLevel = session.thinkingLevel;
		...
		let suffix = 2;
		while (jobs.has(id)) {
			id = `${discoveredJobId(session.path)}-${suffix}`;
			suffix++;
		}
		job = {
			id,
...
async function removeJob(job) {
	...
	if (job.sessionFile && existsSync(job.sessionFile)) {
		job.hidden = true;
		hiddenSessionFiles.add(resolve(job.sessionFile));
		if (job.sessionId) hiddenSessionIds.add(job.sessionId);
		persistJob(job);
	} else {
	...
	switch (message.type) {
		...
		case "rename": {
			...
			if (
				...
				appendNativeSessionName(job.sessionFile, name);
			}
			emitState(job);
			return publicJob(job);
		}
