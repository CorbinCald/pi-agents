...
import { canonicalSessionDirectory } from "../../src/modes/interactive/agents/sessions.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const supervisorPath = join(here, "..", "..", "src", "modes", "interactive", "agents", "supervisor.mjs");
...
test("supervisor discovers, resumes, and retains canonical Pi sessions", {
	timeout: 15_000,
}, async () => {
	const root = mkdtempSync(join(tmpdir(), "pi-agents-canonical-test-"));
		...
		`${[
			...
			},
			{
				...
				message: {
					...
					provider: "fake",
					model: "fake",
					content: [{ type: "text", text: "Existing native session" }],
				},
			},
		]
	...
	try {
		...
		assert.equal(listed[0].discovered, true);

		const prepared = await client.request("prepare_attach", {
			jobId: listed[0].id,
		});
		terminalServer = prepared.terminalServer;
		const launchPath = join(workspace, `launch-${listed[0].id}.json`);
		...
		assert.equal(launch.args.includes("--session-dir"), false);

		await client.request("remove", { jobId: listed[0].id });
