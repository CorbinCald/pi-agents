...
export function supportsMaxProReasoning(
...
export class DispatchReasoningController {
	...
	constructor(
		...
		getModel: () => ReasoningModel | undefined,
	) {
	...
	getSelection(): DispatchReasoningSelection {
		...
		if (
			this.maxProSelected &&
			(thinkingLevel !== "max" || !supportsMaxProReasoning(this.getModel()))
		) {
	...
	cycle(): DispatchReasoningSelection {
		...
		const currentIndex = Math.max(
			0,
			THINKING_LEVELS.indexOf(current.thinkingLevel),
		);
